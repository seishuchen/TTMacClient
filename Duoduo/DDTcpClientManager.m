...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if ([_buffData length] == 0)
    {
        ...
        {
            NSData* dispatchData = [data subdataWithRange:NSMakeRange(0, pduLen)];
            [self p_dispatchData:dispatchData];
            NSData* leftData = [data subdataWithRange:NSMakeRange(pduLen, [data length] - pduLen)];
            [self socket:_socket didReadData:leftData withTag:tag];
            return;
        }
        if (pduLen > [data length])
        {
            ...
            {
                //开始分发
                NSData* dispatchData = [NSData dataWithData:_buffData];
                [self p_dispatchData:dispatchData];
                _buffData = [[NSMutableData alloc] init];
            }
        }
