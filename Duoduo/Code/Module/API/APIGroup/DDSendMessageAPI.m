...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint16_t seqNo)
    {
        ...
        int type = [array[4] intValue];
        
        DataOutputStream *dataout = [[DataOutputStream alloc] init];
        uint32_t totalLen = strLen(fromId) + strLen(toId) + strLen(content) + 16 + 26;
        DDLog(@"  getSendMsgData: 消息长度:%d",totalLen);
        
        [dataout writeInt:totalLen];
        [dataout writeTcpProtocolHeader:MODULE_ID_SESSION cId:CMD_MSG_DATA seqNo:seqNo];
        ...
        [dataout writeInt:0];
        log4CInfo(@"sendMsg serviceID:%i cmdID:%i",MODULE_ID_SESSION,CMD_MSG_DATA);
