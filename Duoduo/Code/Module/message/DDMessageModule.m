...
typedef void(^FetchGroupUnreadMessageCompletion)(NSInteger lastTime);
 ...
 DDMessageModule* getDDMessageModule()
{
    return (DDMessageModule*)[[DDLogic instance] queryModuleByID:MODULE_ID_MESSAGE];
}
...
-(NSArray*)popArrayMessage:(NSString*)sessionId
{
    ...
    @synchronized(self)
    {
        ...
        if(![DDClientState instance].online)
        {
            //用户离线
            if (msgEntityArray)
            {
...
{
    [_sendMsgAckManager pushSendMsgForAck:seqNo message:msg];
}

-(void)pushSendMsgForAck:(uint32)seqNo
{
    [_sendMsgAckManager pushSendMsgForAck:seqNo];
}


-(void)ackSendMsg:(uint32) seqNo
{
    [_sendMsgAckManager ackSendMsg:seqNo];
}

-(BOOL)sendSuccessForMsg:(uint32)seqNo
{
    return [_sendMsgAckManager msgSendSuccess:seqNo];
}

-(NSArray*)getTheLostMsgs
{
    return [_sendMsgAckManager getTheLostMsgs];
}
...
{
    DDUserlistModule* userModule = getDDUserlistModule();
    DDGroupModule* grouModule = getDDGroupModule();
