...
#import "FileTransferWindowController.h"
static CGFloat const itemUpGap = 200;
static CGFloat const animationDuratio = 5;
...
- (void)n_receiveReloginSuccessNotification:(NSNotification*)notification;
...
- (void)awakeFromNib{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(n_receiveReloginSuccessNotification:) name:notificationUserReloginSuccess object:nil];
...
{
    [[FileTransferWindowController instance] showWindow:self];
}
...
-(IBAction)changeOnlineState:(NSMenuItem *)sender{
    ...
    else if(tag==USER_STATUS_OFFLINE)
    {
        ...
        [NotificationHelp postNotification:notificationUserInitiativeOffline userInfo:nil object:nil];
        [stateMaintenanceManager changeMyOnlineState:USER_STATUS_OFFLINE];
    }
}
...
- (void)n_receiveUserLoginSuccessNotification:(NSNotification*)notification
{
    ...
    UserEntity* user = [notification object];
