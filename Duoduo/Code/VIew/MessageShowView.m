...
typedef void(^LoadUserInfoCompletion)(UserEntity* user);
...
//        [self.documentView setFlipped:NO];
        ...
        if ([name length] > 0 && left)
        {
            ...
            if ([name hasPrefix:signString])
            {
                ...
                [self p_loadUserInfo:userID completion:^(UserEntity *user) {
...
- (EGOImageView*)senderAvatarUserID:(NSString*)userID
{
    ...
    UserEntity* user = [getDDUserlistModule() getUserById:userID];
