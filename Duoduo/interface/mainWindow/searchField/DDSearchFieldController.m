...
- (NSArray *)allKeywords
{
    if (allKeywords == nil)
	{
        ...
        allKeywords = [NSMutableArray arrayWithArray:[userListModule getAllUsers]];
...
//    }
    ...
    if([textView.string length] > 0)
    {
        ...
        NSString* searchText = [_searchField.stringValue copy];
        [[DDSearch instance] searchContent:searchText completion:^(NSArray *result, NSError *error) {
            if (!error)
            {
                if (![searchText isEqualToString:_searchField.stringValue])
                {
                    return;
                }
                
                _resultList = result;
                CGFloat height = [_resultList count] * CELL_HEIGHT > RESULT_LIST_MAX_HEIGHT ? RESULT_LIST_MAX_HEIGHT : [_resultList count] * CELL_HEIGHT;
                
                NSRect rect =  [_searchField frame];
                NSPoint pt = NSMakePoint(rect.origin.x, rect.origin.y);
                pt = [[_searchField superview] convertPoint:pt toView:_searchField.superview.superview.superview];
                
                [_scrollView setHeight:height];
                [_scrollView setFrameOrigin:CGPointMake(pt.x, pt.y - height)];
                
                //            [_scrollView setUp:525];j
                
                [_tableView setHeaderView:nil];
                [_tableView reloadData];
                if (![self.tableView target])
                {
                    [self.tableView setTarget:self];
                    [self.tableView setDoubleAction:@selector(doubleClickTableView:)];
                }
            }
        }];
    }
...
- (NSView*)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if ([identifier isEqualToString:@"ResultListColumnIdentifier"])
    {
        ...
        UserEntity* user = _resultList[row];
...
- (void)beginChatWithUser:(id)object
{
    ...
    [[DDMainWindowController instance] openChatViewByUserId:objectID];
    [_scrollView setHidden:YES];
}
